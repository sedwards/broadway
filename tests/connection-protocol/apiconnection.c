...
gboolean
_broadway_server_lookahead_event (BroadwayServer  *server,
                                      const char         *types)
{
...
gulong
_broadway_server_get_next_serial (BroadwayServer *server)
{
  return (gulong)server->next_serial;
}

gboolean
broadway_req_lookahead_event (BroadwayServer  *server,
				      const char         *types)
{
...
guint32
broadway_req_get_last_seen_time (BroadwayServer *server)
{
  return 0;
}

static guint32
broadway_req_send_message_with_size (BroadwayServer *server, BroadwayRequestBase *base,
...
#define broadway_req_send_message(_server, _msg, _type) \
  broadway_req_send_message_with_size(_server, (BroadwayRequestBase *)&_msg, sizeof (_msg), _type)
...
process_input_messages (BroadwayServer *server)
{
  ...
  while (server->incomming)
	...
	g_list_delete_link (server->incomming,
      ...
      {
	//broadway_events_got_input (&reply->event.msg);
	printf("BIG FUCKING FIXME: We do nothing with events yet\n");
      }
      else
...
static BroadwayReply *
broadway_req_wait_for_reply (BroadwayServer *server,
				    guint32 serial)
{
  BroadwayReply *reply;
...
void
broadway_req_flush (BroadwayServer *server)
{
  BroadwayRequestFlush msg;

  broadway_req_send_message(server, msg, BROADWAY_REQUEST_FLUSH);
}

void
broadway_req_sync (BroadwayServer *server)
{
  BroadwayRequestSync msg;
  guint32 serial;
  BroadwayReply *reply;

  serial = broadway_req_send_message (server, msg,
					     BROADWAY_REQUEST_SYNC);
  reply = broadway_req_wait_for_reply (server, serial);
...
void
broadway_req_query_mouse (BroadwayServer *server,
				  guint32            *toplevel,
				  gint32             *root_x,
				  gint32             *root_y,
				  guint32            *mask)
...
guint32
broadway_req_server_new_window (BroadwayServer *server,
				 int x,
				 int y,
				 int width,
				 int height,
				 gboolean is_temp)
{
  ...
  printf("broadway_req_server_new_window - with geometry %d %d %d %d\n",x,y,width,height);
  ...
  printf("broadway_req_server_new_window - sending message\n");
  serial = broadway_req_send_message (server, msg,
					     BROADWAY_REQUEST_NEW_WINDOW);

  printf("broadway_req_server_new_window - waiting for reply\n");
  reply = broadway_req_wait_for_reply (server, serial);
  ...
  printf("broadway_req_server_new_window - complete\n");

  return id;
}

void
broadway_req_server_destroy_window (BroadwayServer *server,
				     gint id)
{
  BroadwayRequestDestroyWindow msg;

  msg.id = id;
  broadway_req_send_message (server, msg,
				    BROADWAY_REQUEST_DESTROY_WINDOW);
}

gboolean
broadway_req_server_window_show (BroadwayServer *server,
				  gint id)
{
...
gboolean
broadway_req_window_hide (BroadwayServer *server,
				  gint id)
{
...
void
broadway_req_server_window_focus (BroadwayServer *server,
				   gint id)
{
...
void
broadway_req_server_window_set_transient_for (BroadwayServer *server,
					       gint id, gint parent)
{
...
void
broadway_req_server_window_update (BroadwayServer *server,
				    gint id,
				    cairo_surface_t *surface)
{
  ...
  data = cairo_surface_get_user_data (surface, &broadway_shm_cairo_key);
  g_assert (data != NULL);
...
gboolean
broadway_req_window_move_resize (BroadwayServer *server,
					 gint id,
					 gboolean with_move,
					 int x,
					 int y,
...
#if 0
GdkGrabStatus
broadway_req_grab_pointer (BroadwayServer *server,
				   gint id,
...
guint32
broadway_req_ungrab_pointer (BroadwayServer *server,
				     guint32    time_)
{
...
void
broadway_req_set_show_keyboard (BroadwayServer *server,
                                        gboolean show)
{
...
broadway_req_screen_init_root_window (GdkScreen * screen)
{
  ...
  //window->visual = screen_get_system_visual (screen);
  ...
  window->width = 1280;
  window->height = 1024;
  //window->width = screen_get_width (screen);
  //window->height = screen_get_height (screen);
  window->viewable = TRUE;

  //_window_update_size (broadway_screen->root_window);
}
