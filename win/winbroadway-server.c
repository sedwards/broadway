...
#include "winbroadway-server.h"
#include "broadway-protocol.h"
void _win_broadway_events_got_input (BroadwayInputMsg *message);
...
static void
win_broadway_server_finalize (GObject *object)
{
  G_OBJECT_CLASS (win_broadway_server_parent_class)->finalize (object);
}
...
win_broadway_server_class_init (WinBroadwayServerClass * class)
{
  ...
  object_class->finalize = win_broadway_server_finalize;
}
...
#define win_broadway_server_send_message(_server, _msg, _type) \
...
process_input_messages (WinBroadwayServer *server)
{
  ...
  while (server->incomming)
	...
	_win_broadway_events_got_input (&reply->event.msg);
      else
...
_win_broadway_server_flush (WinBroadwayServer *server)
{
  ...
  win_broadway_server_send_message(server, msg, BROADWAY_REQUEST_FLUSH);
}
...
_win_broadway_server_sync (WinBroadwayServer *server)
{
  ...
  serial = win_broadway_server_send_message (server, msg,
					     BROADWAY_REQUEST_SYNC);
  reply = win_broadway_server_wait_for_reply (server, serial);
				     ...
				     gint id)
{
  ...
  msg.id = id;
  win_broadway_server_send_message (server, msg,
...
static const cairo_user_data_key_t win_broadway_shm_cairo_key;
...
cairo_surface_t *
_win_broadway_server_create_surface (int                 width,
				     int                 height)
{
  ...
  cairo_surface_set_user_data (surface, &win_broadway_shm_cairo_key,
				    ...
				    cairo_surface_t *surface)
{
  ...
  data = cairo_surface_get_user_data (surface, &win_broadway_shm_cairo_key);
  g_assert (data != NULL);
