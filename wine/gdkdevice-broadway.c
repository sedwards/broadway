				 ...
				 GdkModifierType  *mask)
{
  ...
  wine_broadway_server_query_mouse (broadway_display->server,
				    &mouse_toplevel_id,
				    &device_root_x,
				    &device_root_y,
				    &mask32);
...
void
wine_broadway_window_grab_check_unmap (GdkWindow *window,
				       gulong     serial)
{
...
void
wine_broadway_window_grab_check_destroy (GdkWindow *window)
{
  GdkDisplay *display = gdk_window_get_display (window);
  GdkDeviceManager *device_manager;
			  ...
			  guint32       time_)
{
  ...
  if (gdk_device_get_source (device) == GDK_SOURCE_KEYBOARD)
    ...
    {
      /* Device is a pointer */
      return wine_broadway_server_grab_pointer (broadway_display->server,
			    ...
			    guint32    time_)
{
  ...
  if (gdk_device_get_source (device) == GDK_SOURCE_KEYBOARD)
    ...
    {
      /* Device is a pointer */
      serial = wine_broadway_server_ungrab_pointer (broadway_display->server, time_);
