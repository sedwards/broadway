...
void
wine_broadway_events_got_input (BroadwayInputMsg *message)
{
  GdkDisplay *display;
  ...
  case BROADWAY_EVENT_POINTER_MOVE:
    if (wine_broadway_moveresize_handle_event (display, message))
      break;
  ...
  case BROADWAY_EVENT_BUTTON_RELEASE:
	...
	wine_broadway_moveresize_handle_event (display, message))
      break;
    ...
    if (window)
      {
        ...
        case 2:
          ...
          break;
        default:
          g_printerr ("wine_broadway_events_got_input - Unknown touch type %d\n", message->touch.touch_type);
        }

        if (event_type != GDK_TOUCH_BEGIN &&
            message->touch.is_emulated && wine_broadway_moveresize_handle_event (display, message))
          break;
	...
	if (window->resize_count >= 1)
	  {
	    ...
	    if (window->resize_count == 0)
	      wine_broadway_moveresize_configure_done (display, window);
	  }
      }
    break;
  ...
  default:
    g_printerr ("wine_broadway_events_got_input - Unknown input command %c\n", message->base.type);
    break;
  }
}

void
wine_broadway_display_queue_events (GdkDisplay *display)
{
}
...
GSource *
wine_broadway_event_source_new (GdkDisplay *display)
{
  GSource *source;
